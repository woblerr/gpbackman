...
func TestGetBackupDir(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                "BackupDir is set",
			backupDir:           "/path/to/backup",
			backupDataBackupDir: "",
			backupDataDBName:    "mydb",
			want:                "/path/to/backup",
			wantErr:             false,
		},
		{
			name:                "BackupDataBackupDir is set",
			backupDir:           "",
			backupDataBackupDir: "/path/to/backup",
			backupDataDBName:    "mydb",
			want:                "/path/to/backup",
			wantErr:             false,
		},
		{
			name:                "BackupDir and BackupDataBackupDir are empty",
			backupDir:           "",
			backupDataBackupDir: "",
			backupDataDBName:    "mydb",
			want:                "",
		...
		{
			name:                "BackupDir and BackupDataBackupDir are empty, BackupDirClusterInfo is not found",
			backupDir:           "",
			backupDataBackupDir: "",
			backupDataDBName:    "",
			want:                "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := getBackupDir(tt.backupDir, tt.backupDataBackupDir, tt.backupDataDBName)
			if (err != nil) != tt.wantErr {
				t.Errorf("getBackupDir() error:\n%v\nwantErr:\n%v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("getBackupDir():\n%v\nwant:\n%v", got, tt.want)
			}
		})
	}
}
