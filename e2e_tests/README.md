...
The `backup-info` suite creates local backups for `demo` and an additional database after its timestamp-sensitive cases to cover exact `--database` filtering in normal, combined, timestamp, detail, and empty-result output.
